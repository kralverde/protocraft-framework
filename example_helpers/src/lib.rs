...
use protocraft_framework::{
    ...
    protocol::{Handshake, LegacyPingResponse, versions::v1_21_10},
    traits::{
        EncryptableStreamProvider, Serializable,
        asynchronous::{
            AsyncBoundedReader, AsyncFromReader, AsyncProtocolStateHandler,
            AsyncReadStreamProvider, AsyncToWriter, AsyncWriteStreamProvider, AsyncWriter,
        },
    },
};
use rand::Rng;
...
// A bare-bones server
pub async fn handle_connection_with_errors<P>(
    provider: P,
    ...
    P: AsyncReadStreamProvider + AsyncWriteStreamProvider + EncryptableStreamProvider,
{
        ...
        .map_err(Error::Read)?
    {
        if is_status {
                ...
                ($reason:expr) => {
                    handler
                        .async_write_packet(
                            ...
                            &KickPacket::new($reason).expect("The message is too long!"),
                        )
                        .await
                        .map_err(Error::Write)?;
                    return Ok(());
            ...
            enum RecvStart {}
            impl AsyncProtocolStateHandler for RecvStart {
                type PacketDesignator = v1_21_10::ServerboundLoginPacket;
                    ...
                    R: AsyncBoundedReader,
                {
                    Ok(
                        if matches!(designator, v1_21_10::ServerboundLoginPacket::LoginStart) {
                            let name_length: i32 = VarInt::async_from_reader(reader).await?.into();
                            if name_length < 0 {
            ...
            let Some((name, uuid)) = handler
                .async_read_packet::<RecvStart>()
                .await
                .map_err(Error::Read)?
            else {
            ...
            let (token, rsa_priv) = {
                ...
                let rsa_priv =
                    RsaPrivateKey::new(&mut rng, 1024).expect("Failed to create RSA private key");
            ...
            impl AsyncToWriter for EncryptionRequest<'_> {
                async fn async_to_writer<W>(
                    &self,
                    writer: &mut W,
                ) -> Result<(), WriteError<W::Error>>
                ...
                {
                    // No server ID
                    VarInt::from(0).async_to_writer(writer).await?;
                    VarInt::from(self.public_key.len() as i32)
                        .async_to_writer(writer)
                        .await?;
                    writer
                        .async_write(self.public_key)
                        .await
                        .map_err(WriteError::StreamError)?;
                    VarInt::from(self.token.len() as i32)
                        .async_to_writer(writer)
                        .await?;
                    writer
                        .async_write(self.token)
                        .await
                        .map_err(WriteError::StreamError)?;
                    // Don't authenticate
                    0u8.async_to_writer(writer).await?;
                    Ok(())
                }
            }
            ...
            enum EncryptionResponse {}
            impl AsyncProtocolStateHandler for EncryptionResponse {
                type PacketDesignator = v1_21_10::ServerboundLoginPacket;
                    ...
                    R: AsyncBoundedReader,
                {
                    Ok(
                        if matches!(
                            ...
                            let shared_secret_length: i32 =
                                VarInt::async_from_reader(reader).await?.into();
                            let mut shared_secret = vec![0u8; shared_secret_length as usize];
                            reader
                                .async_read_exact(&mut shared_secret)
                                .await
                                .map_err(ReadError::StreamError)?;
                            let verify_token_length: i32 =
                                VarInt::async_from_reader(reader).await?.into();
                            let mut verify_token = vec![0u8; verify_token_length as usize];
                            reader
                                .async_read_exact(&mut verify_token)
                                .await
                                .map_err(ReadError::StreamError)?;
            ...
            let Some((secret, response_token)) = handler
                .async_read_packet::<EncryptionResponse>()
                .await
                .map_err(Error::Read)?
            else {
            ...
            impl AsyncToWriter for SetCompression {
                async fn async_to_writer<W>(
                    &self,
                    writer: &mut W,
                ) -> Result<(), WriteError<W::Error>>
                where
                    W: AsyncWriter,
                {
                    VarInt::from(1).async_to_writer(writer).await?;
                    Ok(())
                }
            }
            handler
            ...
            impl AsyncToWriter for LoginSuccess {
                async fn async_to_writer<W>(
                    &self,
                    writer: &mut W,
                ) -> Result<(), WriteError<W::Error>>
                ...
                {
                    writer
                        .async_write(self.uuid.as_bytes())
                        .await
                        .map_err(WriteError::StreamError)?;
                    VarInt::from(self.username.len() as i32)
                        .async_to_writer(writer)
                        .await?;
                    writer
                        .async_write(self.username.as_bytes())
                        .await
                        .map_err(WriteError::StreamError)?;
                    VarInt::from(0).async_to_writer(writer).await?;
                    Ok(())
            ...
            handler
                .async_write_packet(
                    ...
                    &LoginSuccess::new(name, uuid),
                )
                .await
                .map_err(Error::Write)?;

            enum LoginAcknowledge {}
            impl AsyncProtocolStateHandler for LoginAcknowledge {
                type PacketDesignator = v1_21_10::ServerboundLoginPacket;
                type Result = bool;
            ...
            if !handler
                .async_read_packet::<LoginAcknowledge>()
                .await
                .map_err(Error::Read)?
            {
            ...
            handler
                .async_write_packet(
                    ...
                    &Nbt(nbt.write_unnamed().into()),
                )
                .await
                .map_err(Error::Write)?;

            enum ConfigPacket {}
            impl AsyncProtocolStateHandler for ConfigPacket {
                type PacketDesignator = v1_21_10::ServerboundConfigPacket;
                type Result = bool;
            ...
            loop {
                if handler
                    .async_read_packet::<ConfigPacket>()
                    .await
                    .map_err(Error::Read)?
                {
                    break;
            ...
            impl AsyncToWriter for ClearDialog {
                async fn async_to_writer<W>(
                    &self,
                    _writer: &mut W,
                ) -> Result<(), WriteError<W::Error>>
        ...
        handler
            .async_write_legacy_ping_response(&response)
            .await
            .map_err(Error::Write)?;
    }
